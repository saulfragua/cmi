<div class="p-4 text-white bg-black min-h-screen">
    ...
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-[#c8982e]">🎓 Panel de Cursos</h1>

        <a href="<?= BASE_URL ?>/cursos/crear"
                        ...
                        <tr class="hover:bg-[#0b1220]">
                            ...
                            <td class="border border-[#2b3d57] px-3 py-2">
                                <?php if (!empty($c['imagen'])): ?>
                                    <img src="<?= BASE_URL ?>/assets/img/cursos/<?= htmlspecialchars($c['imagen']) ?>"
                            ...
                            <td class="border border-[#2b3d57] px-3 py-2">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <a href="<?= BASE_URL ?>/cursos/editar?id=<?= $c['id'] ?>"
                                       class="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 text-xs rounded">
                                        Editar
                                    </a>

                                    <?php if ($c['estado'] == 'Activo'): ?>
                                        <form method="POST" action="<?= BASE_URL ?>/cursos/inactivar">
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="<?= BASE_URL ?>/cursos/activar">
                                            <input type="hidden" name="id" value="<?= $c['id'] ?>">
                                    ...
                                    <form method="POST" action="<?= BASE_URL ?>/cursos/eliminar"

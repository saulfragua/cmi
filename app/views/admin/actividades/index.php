<div class="p-6 text-white bg-[#05070d] min-h-screen">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div class="border-l-4 border-[#c8982e] pl-4">
            ...
            <p class="text-sm text-gray-400">
                Programación operativa por mes y año
            </p>
        </div>

        <div class="flex flex-wrap items-end gap-3">
            <form method="GET" action="<?= BASE_URL ?>/actividades" class="flex flex-wrap items-end gap-3">
                <div>
            ...
            <a href="<?= BASE_URL ?>/actividades/crear"
                class="px-4 py-2 bg-[#c8982e] text-black rounded-lg font-bold hover:opacity-90 transition">
                ➕ Nueva actividad
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-7 gap-2">
        ...
        <?php for ($d = 1; $d <= $diasMes; $d++): ?>
            <?php
            $fechaDia = $anio . '-' . str_pad($mes, 2, '0', STR_PAD_LEFT) . '-' . str_pad($d, 2, '0', STR_PAD_LEFT);
            $tieneActividades = isset($calendario[$d]) && !empty($calendario[$d]);
            ?>

            <div
                onclick="window.location='<?= BASE_URL ?>/actividades/crear?fecha=<?= $fechaDia ?>'"
                class="bg-[#0b1220] border border-[#1f2937] rounded-xl p-2 min-h-[130px] hover:border-[#c8982e] transition cursor-pointer">
